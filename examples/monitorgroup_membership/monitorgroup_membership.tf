# To assign a monitor to a monitor group, you need first to create both the monitor group and the itrs-uptrends_monitor. Then, you can create a `monitorgroup_membership` resource that links the two.
...
import {
  to = itrs-uptrends_monitorgroup_membership.monitorgroup_membership_imported
  id = "${itrs-uptrends_monitor.certificate_monitor.id}:${itrs-uptrends_monitorgroup.monitorgroup_example.id}" # Replace with the actual ID (e.g. "046a727c-7a90-4776-9e41-ab050bdda5dc:046a727c-7a90-4776-9e41-ab050bdda5dc")
  provider          = itrs-uptrends.uptrendsauthenticated
}
