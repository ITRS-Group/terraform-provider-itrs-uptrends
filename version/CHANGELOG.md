...
- New resources: 
  - `itrs-uptrends_escalation_level_integration` — attach integrations (Email, SMS, Phone, Slack, PagerDuty, etc.) to alert definition escalation levels.
  - `itrs-uptrends_alertdefinition_monitorgroup_membership` — manage monitor group memberships for alert definitions.
  - `itrs-uptrends_vault_section_permission` — manage authorization permissions (`ViewVaultSection`, `ChangeVaultSection`) for vault sections, assignable to operators or operator groups.
- New operator resource attributes: `outgoing_phone_number_id`, `time_zone_id`, `setup_mode`, `culture_name`, `use_numeric_sender`, `allow_native_login`, `allow_single_signon`, and their `*_specified` companion flags.
- New operator data source attributes: `outgoing_phone_number_id`, `time_zone_id`.

### Changed

- **Breaking**: `backup_email` on the operator resource changed from Required to Optional+Computed.
- **Breaking**: `mobile_phone` on the operator resource is now Required (empty string accepted).
- **Breaking**: `password_wo` is now conditionally required based on `setup_mode` and `allow_native_login`.
- Five operator fields (`setup_mode`, `culture_name`, `use_numeric_sender`, `allow_native_login`, `allow_single_signon`) are now write-only — sent on create/update but not stored in state.
- Removed the above five fields from the operator data source.

### Fixed
- Fixed monitor and vault item validation incorrectly treating unknown attribute values (e.g. from `for_each` or computed references) as user-provided, which caused false "attribute not allowed" errors during the plan/validate phase.
- Fixed monitor group validation to skip the validate stage entirely when any quota field is unknown, preventing false-positive validation errors.
