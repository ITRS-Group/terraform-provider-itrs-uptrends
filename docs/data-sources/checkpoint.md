...
data "itrs-uptrends_checkpoint" "example" {
  checkpoints = ["Amsterdam", "Cairo", "London"]
}
...
### Read-Only
- `selected_checkpoints_ids` (List of Number) IDs of the checkpoints matching the requested names (or all when none specified). This list is sorted and when used within the `itrs-uptrends_monitor` is going to be compatible with the result of the API to avoid configuration-state inconsistency.
