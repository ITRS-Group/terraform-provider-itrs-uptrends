...
```terraform
resource "monitor" "transaction_monitor" {
  provider = itrs-uptrends.uptrendsauthenticated
...
```terraform
resource "monitor" "multistepapi_monitor" {
  provider = itrs-uptrends.uptrendsauthenticated
...
```terraform
resource "monitor" "mssql_monitor" {
  provider = itrs-uptrends.uptrendsauthenticated
  name     = "MSSQL monitor"
  monitor_type      = "MSSQL"
  generate_alert    = true
...
```terraform
resource "monitor" "mysql_monitor" {
  provider = itrs-uptrends.uptrendsauthenticated
  name     = "MySQL monitor"
  monitor_type      = "MySQL"
  generate_alert    = true
...
```terraform
resource "monitor" "connect_monitor" {
...
> **Note:** Creating a `PostmanApi` monitor via Terraform is not currently supported by the Uptrends API.  
> However, you can **import** existing PostmanApi monitors into your Terraform state and update their common attributes (such as name, check interval, etc.).
>
> - All common attributes are supported for updates.
> - The Postman collection itself **cannot** be viewed or modified using Terraform.
...
terraform import monitor.example "046a727c-7a90-4776-9e41-ab050bdda5dc"
```
