...
func (d *vaultItemDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var data tfsdkmodels.VaultItemResourceModelDataSource
	...
	if idProvided {
		...
		state = converters.UpdateStateConversionDataSource(item)
	} else {
		...
		for idx := range items {
			if strings.EqualFold(items[idx].Name, name) {
				...
				state = converters.UpdateStateConversionDataSource(&items[idx])
				found = true
			}
		}
		if !found {
