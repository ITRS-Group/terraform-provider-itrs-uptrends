...
func (d *monitorDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var data tfsdkmodels.MonitorModelDataSource
	...
	var monitorResponse *tfsdkmodels.MonitorModelDataSource
	...
	if idProvided {
		...
		state := converters.UpdateStateConversionDataSource(monitor)
		...
		for idx := range monitors {
			if strings.EqualFold(monitors[idx].Name, name) {
				...
				state := converters.UpdateStateConversionDataSource(&monitors[idx])
				monitorResponse = &state
				found = true
			}
		}
